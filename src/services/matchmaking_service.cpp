...
MatchmakingService::creator_key MatchmakingService::create(
...
void MatchmakingService::remove_game(std::string game_uuid, std::string creator_key) {
    // запрос в редис на проверку что есть такая игра и что там такой же creator_key
    // запрос на удаление игры из редиса
}


MatchmakingSesion MatchmakingService::join(boost::asio::io_context &io_context, const std::string &creator_uuid,
